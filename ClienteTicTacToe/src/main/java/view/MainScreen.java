...
public final class MainScreen extends javax.swing.JFrame {
    ...
    private void jButtonStartGameMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButtonStartGameMouseClicked
        try {
            ...
            String oponent = jTablePlayers.getValueAt(selectedRow, 0).toString();

            API api = new API();
            ResponseModel response = api.inviteGame(this.getNickname(), oponent);

            InfoDialog info_window = new InfoDialog();
            if (response.getResponseCode() == 200) {
                info_window.SetMessage("Convite enviado!");
            } else {
                info_window.SetMessage("Erro ao se comunicar com servidor!");
            }
        } catch (Exception ex) {
            System.out.println("TESTEEEEEEE" + ex);
        }
